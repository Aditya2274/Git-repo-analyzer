...
# ----------------------------------------
data = run("GIT_EDITOR=true git shortlog -s -n --all")
if data:
    authors, commits = [], []
    ...
    plt.tight_layout()
    plt.savefig(f"{CHART_DIR}/commits_per_author.png")
    plt.close()
...
if dates_raw:
    ...
    plt.tight_layout()
    plt.savefig(f"{CHART_DIR}/daily_commit_activity.png")
    plt.close()
...
show_commit_summary() {
  ...
  GIT_EDITOR=true git shortlog -s -n --all 2>/dev/null
}
...
# Author chart
raw = run("GIT_EDITOR=true git shortlog -s -n --all")
if raw:
    authors, counts = [], []
